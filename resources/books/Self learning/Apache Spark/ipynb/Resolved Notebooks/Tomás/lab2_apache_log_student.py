...
APACHE_ACCESS_LOG_PATTERN = '^(\S+) (\S+) (\S+) \[([\w:/]+\s[+\-]\d{4})\] "(\S+) (\S+)\s*(\S*)\s*" (\d{3}) ([\d-]+)'
parsed_logs, access_logs, failed_logs = parseLogs()
...
endpointCountPairTuple = not200.map(lambda r: (r.endpoint, 1))

endpointSum = endpointCountPairTuple.reduceByKey(lambda x, y: x + y).cache()
...
uniqueHosts = hosts.distinct().cache()
...
dayHostCount = dayGroupedHosts.map(lambda l: (l[0], len(set(l[1]))))
...
avgDailyReqPerHost = (sortedByDay.map(lambda l: (l[0], len(l[1])/len(set(l[1]))))).cache()
...
badUniqueEndpoints = badEndpoints.distinct().cache()
...
badEndpointsSum = badEndpointsCountPairTuple.reduceByKey(lambda x, y: x + y).cache()
...
errHostsSum = errHostsCountPairTuple.reduceByKey(lambda x, y: x + y).cache()
