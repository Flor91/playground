...
def simpleTokenize(string):
    ...
    """
    sanitized = re.sub('[.!/]', ' ', string)
    if sanitized == ' ': return []
    else: process = sanitized.strip().lower()
    return filter(lambda x: x != '', re.split(split_regex, process))
...
def countTokens(vendorRDD):
    ...
    """
    return vendorRDD.map(lambda t: len(t[1])).reduce(lambda x, y: x + y)
...
print amazonRecToToken.takeOrdered(1, lambda s: -1 * len(s[1]))[0][1]
...
def tf(tokens):
    ...
    for word in tokens:
        ...
        count += 1
    return dict(map(lambda (t, v): (t, float(v) / count), dic.iteritems()))
...
def idfs(corpus):
    ...
    """
    N = corpus.count()
    uniqueTokens = corpus.flatMap(lambda t: set(t[1]))
    tokenCountPairTuple = uniqueTokens.map(lambda word: (word, 1))
...
def tfidf(tokens, idfs):
    ...
    """
    tfs = tf(tokens)
    tfIdfDict = dict(map(lambda (token, tf): (token, tf * idfs[token]), tfs.iteritems()))
    return tfIdfDict
...
# TODO: Replace <FILL IN> with appropriate code
sims = similaritiesBroadcast.map(lambda (g,a,s): (a + ' ' + g, s)).cache()

trueDupsRDD = (sims.join(goldStandard)
               .map(lambda (u, s): s[0])).cache()
trueDupsCount = trueDupsRDD.count()
avgSimDups = trueDupsRDD.sum() / float(trueDupsCount)

nonDupsRDD = (sims.subtract(sims.join(goldStandard)
                            .map(lambda (u, s): (u, s[0])))
              .map(lambda (u, s): s)).cache()
avgSimNon = nonDupsRDD.reduce(lambda x, y: x + y) / nonDupsRDD.count()
...
print sims.join(goldStandard).map(lambda (u, s): (u, s[0])).first()
...
idfsFullBroadcast = sc.broadcast(dict(idfsFullWeights.collect()))
...
# TODO: Replace <FILL IN> with appropriate code
amazonNorms = amazonWeightsRDD.map(lambda (k, d): (k,norm(d))).collect()
amazonNormsBroadcast = sc.broadcast(dict(amazonNorms))
googleNorms = googleWeightsRDD.map(lambda (k, d): (k,norm(d))).collect()
googleNormsBroadcast = sc.broadcast(dict(googleNorms))
...
def invert(record):
    ...
    """
    return [(token , record[0]) for token in record[1].keys()]

amazonInvPairsRDD = (amazonWeightsRDD
                    .flatMap(lambda (i, dic): invert((i, dic)))
                    .cache())
...
def fastCosineSimilarity(record):
    ...
    token = record[1]
    s = amazonWeightsBroadcast.value.get(amazonRec, 0).get(token, 0) * googleWeightsBroadcast.value.get(googleRec, 0).get(token, 0)
    value = s/(googleNormsBroadcast.value.get(googleRec, 1))/(amazonNormsBroadcast.value.get(amazonRec, 1))
...
amazonWeightsBroadcast = sc.broadcast(amazonWeightsRDD.collectAsMap())
googleWeightsBroadcast = sc.broadcast(googleWeightsRDD.collectAsMap())
print commonTokens.top(10)#googleWeightsBroadcast.value.get('http://www.google.com/base/feeds/snippets/13159207338580799968')#map(fastCosineSimilarity, commonTokens.first())
