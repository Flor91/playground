...
# sortBy un string solo resultado de la combinacion
...
def getCountsAndAverages(IDandRatingsTuple):
    ...
    return (IDandRatingsTuple[0], (len(ratings), sum(ratings) / float(len(ratings))))
...
movieIDsWithAvgRatingsRDD = movieIDsWithRatingsRDD.map(getCountsAndAverages).cache()
...
movieNameWithAvgRatingsRDD = (moviesRDD
                              ...
                              .map(lambda (i, (name, (ratings, avg))): (avg, name, ratings))).cache()
...
movieLimitedAndSortedByRatingRDD = (movieNameWithAvgRatingsRDD
                                    .filter(lambda (avg, name, ratings): ratings > 500)
...
def computeError(predictedRDD, actualRDD):
    ...
    squaredErrorsRDD = (predictedReformattedRDD
                        ...
                        .map(lambda (key, (pred, act)): (pred - act)**2))

    # Compute the total squared error - do not use collect()
    totalError = squaredErrorsRDD.sum()

    # Count the number of entries for which you computed the total squared error
    numRatings = float(squaredErrorsRDD.count())
...
trainingAvgRating = trainingRDD.map(lambda (user, movie, avg): avg).sum() / trainingRDD.count()
print 'The average rating for movies in the training set is %s' % trainingAvgRating

testForAvgRDD = testRDD.map(lambda (user, movie, avg): (user, movie, trainingAvgRating))
...
myUnratedMoviesRDD = (moviesRDD
                      ...
                      .filter(lambda x: x[1] not in [i[1] for i in myRatedMovies])).cache()
...
movieCountsRDD = movieIDsWithAvgRatingsRDD.map(lambda (movie, (n, avg)): (movie, n))

# Transform predictedRatingsRDD into an RDD with entries that are pairs of the form (Movie ID, Predicted Rating)
predictedRDD = predictedRatingsRDD.map(lambda r: (r[1], r[2]))
...
ratingsWithNamesRDD = (predictedWithCountsRDD
                       .join(moviesRDD)
                       .map(lambda (movie, ((avg, n), desc)): (avg, desc, n)))
